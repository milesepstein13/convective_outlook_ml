...
def run_crossval(X, y, stats, model_name, n_splits=5, batch_size=64, epochs=5, optimizer_class=torch.optim.Adam, lr=1e-3, criterion=nn.MSELoss(), level=None, restart = False, force_cpu = False):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(days)):
        ...
        if restart:
            # Remove model checkpoint folder for this fold
            if os.path.exists(model_dict):
                shutil.rmtree(model_dict)

            # Remove TensorBoard log folder for this fold
    ...
    avg_writer = SummaryWriter(log_dir=avg_log_dir)
    ...
    for epoch, loss in enumerate(avg_train_curve):
        avg_writer.add_scalar("Loss/train", loss, epoch)
    for epoch, loss in enumerate(avg_val_curve):
        avg_writer.add_scalar("Loss/val", loss, epoch)
